...
const AdminDashboard = () => {
  ...
  const deleteUser = async (id) => {
    if (window.confirm("Are you sure you want to delete this user?")) {
      try {
        //used template literal $ since the id is dynamic
        await axios.delete(`http://localhost:3000/users/${id}`);
        //we can either call the getUsers once again or use like nere .Just delete the id .Getusers is better
  ...
  useEffect(() => {
    getUsers();
  }, []); // empty array to load only when the components loads

  return (
    <div className="container my-5">
      <h3 className="text-center text-white mb-4">Admin Dashboard</h3>
      <div className="row justify-content-center">
        {/* giving an editing feature also using a ternary operator  */}
        {users.map((user) => (
                  ...
                  <>
                    ...
                    <div className="d-flex justify-content-center gap-2">
                      ...
                      <button
                        ...
                        onClick={() => deleteUser(user._id)} // to get the id of the user that we want to delete
                      >
                        Delete
                      </button>
                    </div>
